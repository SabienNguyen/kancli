...
func newColumnForm(col *Column, st styles) columnForm {
	name := textinput.New()
	name.Prompt = ""
...
func (f columnForm) color() string {
	if f.colorIdx >= 0 && f.colorIdx < len(columnPalette) {
		return columnPalette[f.colorIdx]
	}
	return f.custom
}
