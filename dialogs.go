...
func boardItems(f *File) []pickItem {
	items := make([]pickItem, 0, len(f.Boards))
	for _, b := range f.Boards {
		live := len(b.Live())
		desc := fmt.Sprintf("%d task%s, %d column%s", live, plural(live), len(b.Columns), plural(len(b.Columns)))
...
func archiveItems(b *Board, g glyphs) []pickItem {
	var items []pickItem
	for _, t := range b.ArchivedTasks() {
		...
		items = append(items, pickItem{
			num:   t.ID,
			title: t.Ref() + " " + t.Title,
			desc:  "archived " + relTime(*t.ArchivedAt, timeNow()) + " " + g.dot + " from " + col,
		})
	}
	return items
}
