...
func (d cardDelegate) metaLine(t board.Task, width int) string {
	th := d.st.th
	var parts []string
	if label, state := board.DueInfo(t, d.now); label != "" {
...
func (c *column) configure(col board.Column, count int, exceeded bool, d cardDelegate) {
...
func (c *column) setTasks(tasks []board.Task) tea.Cmd {
...
func (c column) selected() (board.Task, bool) {
