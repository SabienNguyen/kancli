...
func newTaskForm(mode formMode, t board.Task, colName string, st Styles, g Glyphs) taskForm {
	...
	case tea.KeyMsg:
		...
		if f.field == fieldPriority {
			...
			case "left", "h", "-", "shift+tab":
				f.priority = (f.priority + board.NumPriorities - 1) % board.NumPriorities
			case "right", "l", "+", " ", "=":
				f.priority = (f.priority + 1) % board.NumPriorities
			case "0", "1", "2", "3", "4":
				f.priority = board.Priority(msg.String()[0] - '0')
			}
			return f, nil
		}
		var cmd tea.Cmd
...
func (f taskForm) submit() (taskForm, tea.Cmd) {
	...
	due, err := board.ParseDue(f.due.Value(), board.Now())
	if err != nil {
		f.err = err.Error()
	...
	t.Due = due
	t.Labels = board.ParseLabels(f.labels.Value())
...
func (f taskForm) View() string {
	...
	// Priority selector.
	var prio []string
	for p := board.Priority(0); p < board.NumPriorities; p++ {
		name := p.String()
		if p == f.priority {
	...
	if v := strings.TrimSpace(f.due.Value()); v != "" {
		if d, err := board.ParseDue(v, board.Now()); err != nil {
			dueLine += "  " + f.st.err.Render("?")
		} else if d != "" {
			t, _ := time.ParseInLocation(board.DateLayout, d, time.Local)
			label, state := board.DueInfo(board.Task{Due: d}, board.Now())
