...
func (v *statsView) Load(b *board.Board, s *store.Store, now time.Time) {
...
func (v *statsView) render() {
	...
	if len(s.Finished) > 0 {
		out = append(out, st.muted.Render("cycle time ")+st.strong.Render(board.HumanDuration(s.CycleMedian))+
			st.muted.Render(" median "+v.g.dot+" "+board.HumanDuration(s.CycleMean)+" mean "+v.g.dot+" "+board.HumanDuration(s.CycleP90)+" p90"))
	} else {
	...
	if len(bars) > 0 {
		...
		for _, cs := range s.Stays {
			if cs.Samples > 0 {
				legend = append(legend, fmt.Sprintf("%s %s (%d)", cs.Name, board.HumanDuration(cs.Mean), cs.Samples))
			}
		}
		if len(legend) > 0 {
	...
	if len(s.Aging) > 0 {
		...
		for _, a := range s.Aging {
			out = append(out, fmt.Sprintf("  %s  %s %s %s", st.warning.Render(fmt.Sprintf("%8s", board.HumanDuration(a.Age))), st.muted.Render(fmt.Sprintf("#%d", a.ID)), a.Title, st.muted.Render("in "+a.Column)))
		}
		out = append(out, "")
	}
	...
	if len(s.Finished) > 0 {
		...
		for i, ft := range s.Finished {
			...
			out = append(out, fmt.Sprintf("  %s  %s %s %s", st.muted.Render(ft.DoneAt.Format("Jan 02")), st.muted.Render(fmt.Sprintf("#%d", ft.ID)), ft.Title, st.success.Render(board.HumanDuration(ft.Cycle))))
		}
	}
	v.vp.SetContent(strings.Join(out, "\n"))
}
...
func colColorByID(v *statsView, id string) string {
	...
	for i, cs := range v.stats.Stays {
		if cs.Column == id {
			return board.ColumnPalette[i%len(board.ColumnPalette)]
		}
	}
	return ""
}

func sumCreated(weeks []board.WeekCount) int {
	n := 0
	for _, w := range weeks {
		n += w.Created
	}
